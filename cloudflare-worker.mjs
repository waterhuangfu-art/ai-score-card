...
function buildBusinessBody(payload) {
  const name = cleanText(payload?.name, 24);
  if (!name) throw new Error('姓名不能为空');
...
async function handleSubmitBusiness(request, url, env) {
  let payload;
  try {
    payload = await parseBody(request);
  } catch {
    return respond(request, url, {}, 'business-submit-result', 400, { success: false, error: '请求格式错误' });
  }

  const ip = getClientIp(request);
  if (isRateLimited(businessRateLimit, ip)) {
    return respond(request, url, payload, 'business-submit-result', 429, { success: false, error: '提交太频繁，请稍后再试' });
  }

  const token = env.GITHUB_TOKEN;
  if (!token) {
    return respond(request, url, payload, 'business-submit-result', 500, { success: false, error: '服务器未配置，请联系管理员' });
  }

  try {
    const repo = env.REPO || DEFAULT_REPO;
    const result = buildBusinessBody(payload);
    await ensureLabel(token, repo, 'business-workshop-card', '0ea5e9');
    for (const label of result.labels.slice(1)) {
      await ensureLabel(token, repo, label, '2563eb');
    }

    const issue = await createIssue(token, repo, result.title, result.body, result.labels);
    return respond(request, url, payload, 'business-submit-result', 200, {
      success: true,
      issueNumber: issue.number
    });
  } catch (error) {
    return respond(request, url, payload, 'business-submit-result', 500, {
      success: false,
      error: error.message || '提交失败'
    });
  }
...
async function handleResultsBusiness(url, env) {
  const token = env.GITHUB_TOKEN || '';
  const repo = env.REPO || DEFAULT_REPO;
  ...
  try {
    ...
    return json({ session, count: records.length, records });
  } catch (error) {
    return json({ error: error.message || '获取后台数据失败' }, 500);
  }
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET') {
      ...
      if (url.pathname === '/api/results-business') {
        return handleResultsBusiness(url, env);
      }
    ...
    if (url.pathname === '/api/submit-business') {
      return handleSubmitBusiness(request, url, env);
    }
