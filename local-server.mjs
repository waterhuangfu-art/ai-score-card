...
const REPO = process.env.REPO || 'waterhuangfu-art/ai-score-card';
...
function respond(res, req, url, payload, messageType, status, data) {
  if (wantsIframeResponse(url)) {
    ...
    return sendHtml(res, status, `<!doctype html>
<html lang="zh-CN">
...
async function handleResultsAi(req, res, url) {
  if (!GITHUB_TOKEN) {
  ...
  try {
    const issues = await listIssues('ai-score-card');
    ...
    return sendJson(res, 200, buildResultsPayload(records, [
...
async function handleResultsOpc(req, res, url) {
  if (!GITHUB_TOKEN) {
    return sendJson(res, 500, { error: '本机未找到 GitHub Token，请先执行 gh auth login' });
  }
  ...
  try {
    const issues = await listIssues('opc-self-card');
    const records = issues
...
function buildAiRecord(payload) {
  const dimensions = [
  ...
  const stage = cleanText(payload?.stage, 20) || (total <= 9 ? '尝鲜期' : total <= 14 ? '工具期' : total <= 19 ? '系统期' : '杠杆期');
...
function buildOpcRecord(payload) {
  const dimensions = [
  ...
  const total = Object.values(scores).reduce((sum, score) => sum + score, 0);
  const stage = total <= 9 ? '观望期' : total <= 14 ? '准备期' : total <= 19 ? '启动期' : '加速期';
  const name = cleanText(payload?.name, 24);
  if (!name) throw new Error('姓名不能为空');
...
## 最想突破
${breakthroughs.length ? breakthroughs.map((item) => `- ${item}`).join('\n') : '（未填写）'}
...
async function handleSubmitAi(req, res, url) {
  ...
  try {
    ...
    return respond(res, req, url, {}, 'ai-submit-result', 400, { success: false, error: '请求格式错误' });
  }

  if (!GITHUB_TOKEN) {
    return respond(res, req, url, payload, 'ai-submit-result', 500, { success: false, error: '本机未找到 GitHub Token，请先执行 gh auth login' });
  }

  const ip = getClientIp(req);
  if (isRateLimited(aiRateLimit, ip)) {
    return respond(res, req, url, payload, 'ai-submit-result', 429, { success: false, error: '提交太频繁，请稍后再试' });
  }

  try {
    const result = buildAiRecord(payload);
    await ensureLabel('ai-score-card', '0ea5e9');
    await ensureLabel(result.stage, { '尝鲜期': 'f59e0b', '工具期': '3b82f6', '系统期': '10b981', '杠杆期': '8b5cf6' }[result.stage] || '94a3b8');
    ...
    return respond(res, req, url, payload, 'ai-submit-result', 200, {
      success: true,
      total: result.total,
      stage: result.stage,
...
async function handleSubmitOpc(req, res, url) {
  let payload;
  try {
    payload = await readBody(req);
  } catch {
    return respond(res, req, url, {}, 'opc-submit-result', 400, { success: false, error: '请求格式错误' });
  }

  if (!GITHUB_TOKEN) {
    return respond(res, req, url, payload, 'opc-submit-result', 500, { success: false, error: '本机未找到 GitHub Token，请先执行 gh auth login' });
  }

  const ip = getClientIp(req);
  if (isRateLimited(opcRateLimit, ip)) {
    return respond(res, req, url, payload, 'opc-submit-result', 429, { success: false, error: '提交太频繁，请稍后再试' });
  }

  try {
    const result = buildOpcRecord(payload);
    await ensureLabel('opc-self-card', '0ea5e9');
    await ensureLabel(result.stage, { '观望期': 'f59e0b', '准备期': '3b82f6', '启动期': '10b981', '加速期': '8b5cf6' }[result.stage] || '94a3b8');
    for (const label of result.labels.slice(2)) {
      await ensureLabel(label, '2563eb');
    }

    const issue = await createIssue(result.title, result.body, result.labels);
    return respond(res, req, url, payload, 'opc-submit-result', 200, {
      success: true,
      total: result.total,
      stage: result.stage,
      issueNumber: issue.number
    ...
    return respond(res, req, url, payload, 'opc-submit-result', 500, {
      success: false,
      error: error.message || '提交失败'
    });
  }
...
async function handleManage(req, res, payloadBuilder, issuePrefix, baseLabel, stageColors) {
  let payload;
  try {
    payload = await readBody(req);
  } catch {
  ...
  if (!GITHUB_TOKEN) {
    return sendJson(res, 500, { success: false, error: '本机未找到 GitHub Token，请先执行 gh auth login' });
  }

  const issueNumber = Number(payload?.issueNumber);
  if (!Number.isInteger(issueNumber) || issueNumber <= 0) {
    return sendJson(res, 400, { success: false, error: 'issueNumber 无效' });
  }

  try {
    if (payload?.action === 'delete') {
      await updateIssue(issueNumber, { state: 'closed' });
      return sendJson(res, 200, { success: true, issueNumber, deleted: true });
    }

    if (payload?.action !== 'update') {
      return sendJson(res, 400, { success: false, error: '不支持的操作' });
    }

    const next = payloadBuilder(payload);
    await ensureLabel(baseLabel, '0ea5e9');
    await ensureLabel(next.stage, stageColors[next.stage] || '94a3b8');
    for (const label of next.labels.slice(2)) {
      await ensureLabel(label, '2563eb');
    }
    ...
    return sendJson(res, 500, { success: false, error: error.message || '操作失败' });
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/api/submit-ai') {
    return handleSubmitAi(req, res, url);
  }

  if (req.method === 'POST' && url.pathname === '/api/submit-opc') {
  ...
  if (req.method === 'POST' && url.pathname === '/api/manage-opc') {
    return handleManage(req, res, buildOpcRecord, '[OPC自评]', 'opc-self-card', {
      '观望期': 'f59e0b',
      '准备期': '3b82f6',
      '启动期': '10b981',
      '加速期': '8b5cf6'
  ...
  if (req.method === 'GET' && url.pathname === '/api/results-ai') {
    return handleResultsAi(req, res, url);
  }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`评分卡本地服务已启动: http://127.0.0.1:${PORT}`);
  for (const ip of getLanAddresses()) {
    console.log(`局域网可访问: http://${ip}:${PORT}`);
  }
  if (!GITHUB_TOKEN) {
    console.log('警告: 未找到 GITHUB_TOKEN。请先执行 gh auth login，或在启动前设置 GITHUB_TOKEN 环境变量。');
  }
});
